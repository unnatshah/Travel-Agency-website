...
$check_pack = "select * from cart where ip_add='$ip' and
p_id='$pack_id'";
$run_check = mysqli_query($con, $check_pack);
if (mysqli_num_rows($run_check) > 0) {
echo "";
} else {
$insert_pack = "insert into cart (p_id, ip_add) values
...
echo "<li><a href='index.php?cat=$cat_id' style='text-
decoration: none;'>$cat_title</a></li>";
...
echo "<li><a href='index.php?type=$type_id' style='text-
decoration: none;'>$type_title</a></li>";
...
if (!isset($_GET['type'])) {
global $con;
$get_pack = "select * from packages order by RAND() LIMIT
...
<h3 style='font-family: Cambria; margin-bottom:

2px;'>$pack_title</h3>

<img src='admin_area/package_images/$pack_image'

width='180' height='180'>

<p><b>Cost: Rs. $pack_price</b></p>
<a href='details.php?pack_id=$pack_id' style='float:

left;font-size:18px;text-decoration: none;'>Details</a>
<a href='index.php?add_cart=$pack_id'><button
...
$cat_id = $_GET['cat'];
global $con;
$get_cat_pack = "select * from packages where
package_cat='$cat_id'";
$run_cat_pack = mysqli_query($con, $get_cat_pack);
$count_cats = mysqli_num_rows($run_cat_pack);
if ($count_cats == 0) {

echo "<h2 style='padding=20px;'>No packages were found in
...
<div id='single_package'>
<h3>$pack_title</h3>
<img src='admin_area/package_images/$pack_image' width='180'
...
<a href='index.php?pack_id=$pack_id'><button style='float:
right; font-size:14px; cursor: pointer; padding: 2px
4px;'>Book</button></a>
</div>
";
}
...
$type_id = $_GET['type'];
global $con;
$get_type_pack = "select * from packages where
package_type='$type_id'";
$run_type_pack = mysqli_query($con, $get_type_pack);
$count_types = mysqli_num_rows($run_type_pack);
if ($count_types == 0) {
echo "<h2 style='padding=20px;'>No packages were found

associated with this type!</h2>";
